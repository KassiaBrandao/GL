...
@app.post("/dramas", response_model=schemas.SuccessResponse[schemas.DramaGL], status_code=201)
...
@app.get("/dramas", response_model=List[schemas.DramaGL])
def read_dramas(
    skip: int = 0,
    limit: int = 10,
...
@app.get("/dramas/{drama_id}", response_model=schemas.DramaGL)
...
@app.put("/dramas/{drama_id}", response_model=schemas.SuccessResponse[schemas.DramaGL])
def update_drama(
...
@app.delete("/dramas/{drama_id}", response_model=schemas.SuccessResponse[schemas.DramaGL])
...
@app.post("/dramas/{drama_id}/episodes", response_model=schemas.SuccessResponse[schemas.Episode])
...
@app.put("/dramas/{drama_id}/episodes/{episode_number}", response_model=schemas.SuccessResponse[schemas.Episode])
def update_episode(
    drama_id: int,
    episode_number: int,
...
@app.delete(
    "/dramas/{drama_id}/episodes/{episode_number}", response_model=schemas.SuccessResponse[schemas.Episode]
)
...
@app.get("/dramas/export/json")
def export_dramas_to_json(db: Session = Depends(get_db)):
    dramas = crud.get_dramas(db, limit=1000)  # Limite alto para exportar "todos"
